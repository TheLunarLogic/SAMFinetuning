...
def calculate_class_instances(data_yaml_path):
    ...
    dataset_path = os.path.join(os.path.dirname(data_yaml_path), data['data'])
    dataset_path = dataset_path.replace("images", "labels")
    total_counts.update(_process_dataset_path(dataset_path))
...
def _setup_new_directory(original_dir, new_dataset_name):
    ...
    for subdir in ['images', 'labels']:
        os.makedirs(os.path.join(new_dir, 'combined', subdir), exist_ok=True)
    return new_dir
...
def _process_label_file(src_path, dst_path, old_to_new_class_map):
    """
    Processes YOLO annotation files, updates class IDs using the new mapping,After processing a YOLO annotation file, 
    it only saves the file to the new dataset if it contains annotations with class IDs that are present in the old_to_new_class_map.

    Args:
        src_path (str): Path to the source annotation file.
        dst_path (str): Path to save the processed annotation file.
        old_to_new_class_map (dict): Mapping of old class IDs to new class IDs.

    Returns:
        bool: True if the file contains at least one valid annotation and is saved; False otherwise.
    """
    keep_file = False
    new_annotations = []
...
def create_yolo_subdataset(data_yaml_path: str, filtered_class_names: list[str], new_dataset_name: str):
    ...
    # Process files
    labels_src_dir = os.path.join(original_dir, 'combined', 'labels')
    labels_dst_dir = os.path.join(new_dir, 'combined', 'labels')
    images_src_dir = os.path.join(original_dir, 'combined', 'images')
    images_dst_dir = os.path.join(new_dir, 'combined', 'images')

    for label_file in tqdm(os.listdir(labels_src_dir), desc='Processing files'):
...
data_yaml_path = '/home/hp-atc/labellerr/MLTraining/kapsys-batch-1-2-yolo/data.yaml'
...
create_yolo_subdataset(data_yaml_path, filtered_class_names, 'filtered')
